...
| Final-review amendments (2026-07-22, recorded post-execution) | (a) **Unit-test baseline corrected**: of the "2 pre-existing failures", the priceVariantPool one was CARVE-CAUSED (its `improve-v1/` fixtures were excluded by the copy filter) — fixtures restored, suite fully green; true baseline = 1 genuine pre-existing failure (universalSafety harassment false-positive). (b) **DoD §6.3 substitution**: the shipped fb-palm Playwright configs target deployed origins (fingershape→prod, hand-size→dev; attribution-key-gated asserts) — the LOCAL gate is the new `tests/standalone-funnel.spec.ts` bridge walk (4/4). (c) **JWT_SECRET kept** (contra §4): consumed by `healthCheck.ts` as an `/api/ready` readiness gate only; no JWTs issued/verified; documented in `.env.example`. (d) Seed writes model-config keys (`default_conversation_model`, `default_basic_model`) in addition to `v1_price_variants` — `modelConfig.ts` reads them. (e) **Push shape**: per this spec's "fresh history, one initial commit" mandate, the PUSHED repo gets a single squashed initial commit of the final tree (also yields a lean pack with zero V2 code in any pushed history); the 12-commit carve chain is retained locally as branch `carve-history`, and this source repo + `.superpowers/sdd/` reports remain the archaeological record |
