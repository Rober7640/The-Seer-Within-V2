...
- **Auth check (new)** — gates both `GET /e/:code` and direct visits to `/aiden`, `/evelyn`, `/luna`. Reads a first-party, server-set, HttpOnly/Secure/SameSite=Lax session cookie (the same model the app's login flow already establishes — not a client-held JWT, which a bare `GET` can't observe). Eligible states: authenticated AND email-verified only — an authenticated-but-unverified session does not qualify (see Decisions) and falls through to the normal anonymous/verify-resume path. When it qualifies: resolve the code's full content row (same single lookup as above, not a second campaign-only lookup) and start a fresh session with `/chat/{personaSlug}` — the CLICKED persona, overriding whatever persona the account was last active with (starting this session ends whatever else was active, exactly like every chat start already does today — the app has no concept of multiple concurrent sessions). `generateGreeting` fires with the arrival-reading context injected, the same mechanism used for any persona's first-ever message, now campaign-aware — and if prior history exists with this persona, the greeting is informed by it too (extending the existing pattern that already restores continuity data like birth-chart info into a new session with a returning persona). There is no separate "quiet, no message" case — every tier-1 arrival gets a visible continuation message, regardless of chat history. Context is never written back as a persisted "current campaign" field on the account, so the check stays safe against prefetching/bots. If the code doesn't resolve for an authenticated visitor: start the session with a neutral, non-campaign-specific greeting instead of erroring. If the code doesn't resolve to even a persona: redirect to `/personas`.
...
flowchart TD
  ...
  G1 --> G3["Start fresh session at<br/>/chat/personaSlug (ends any<br/>other active session, as always)"]
  G3 --> G4["FRAME 0 — generateGreeting fires<br/>with arrival-reading context<br/>(+ prior-persona history if any) —<br/>same behavior regardless of history"]
...
- Already-authenticated-and-verified skip path: such a reader hitting `/e/:code` OR visiting `/aiden`/`/evelyn`/`/luna` directly never sees the lander and lands directly in `/chat/{personaSlug}` with a fresh `generateGreeting` call carrying the campaign's arrival-reading context — confirm this fires identically whether or not prior history exists with that persona (no separate "quiet" behavior), and that starting this session correctly ends whatever other session was previously active.
