...
- **Port-as-is.** Only four behavior deviations are authorized: (1) experiments framework absent → woven arm unassigned → control default (client fetch left untouched; `?clearing=woven` override still works); (2) SuccessPage Luna cross-sell removed; (3) `/` redirects to `/fb-palm`; (4) **[added during Task 6, controller-authorized]** `/api/metrics` (healthCheck `getMetrics`) drops its three V2 metrics (users / chat sessions / credit purchases — tables that would be forever-empty in the standalone), enabling the intended 4-table schema; facebook.ts's dormant zero-caller legacy functions referencing those tables are deleted as orphans. Any other behavior change is a bug.
