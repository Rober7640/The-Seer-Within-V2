#!/usr/bin/env python3
"""⛔⛔ RETIRED — DO NOT RUN. Superseded by scripts/build-07-daily-v2.py (2026-09-04).

WHY IT IS RETIRED
  It keeps the copy in a Python dict, so shipping approved prose through it means re-keying
  every sentence by hand — the one operation that can silently reword a letter the operator
  signed off. It also has no slot for the face-down crop or the position-label table, and it
  points at the retired marcus/card-back.jpg. Running it REGRESSES all seven emails.

WHAT TO USE
  scripts/build-07-daily-v2.py — reads the approved .md, so a rebuild cannot disagree with
  the file that was approved. Verified: every word of all seven .md survives into the .html.
"""
import sys
sys.exit("build-07-daily.py is RETIRED and regresses all seven emails. "
         "Use scripts/build-07-daily-v2.py, which builds from the approved .md.")

# ── the retired implementation, kept for reference ──
#
