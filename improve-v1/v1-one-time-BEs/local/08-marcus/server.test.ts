...
/** The checkout stand-in's payload: email + name on card (display) + the two custom fields. */
const payer=(over:Record<string,unknown>={})=>({email:'test@example.test',displayFirstName:'Joel',fullBirthName:'Joel Chue',dateOfBirth:'1961-03-14',...over});
...
test('local journey preserves context, prices, draws and replay behavior',async()=>{
 ...
 try{
  ...
  assert.equal((await call('/api/intake',{editionId:'healing-v1',sameDay:'yes'})).status,400);
  assert.equal((await call('/api/intake',{editionId:'nope',sameDay:false})).status,400);
  ...
  const intake=(await call('/api/intake',{editionId:'healing-v1',sameDay:true,priceCents:1,firstName:'ignored'})).data.intake;
  ...
  assert.equal((await call('/api/local-pay',{intakeId:intake.id,...payer({displayFirstName:' '})})).status,400);
  assert.equal((await call('/api/local-pay',{intakeId:intake.id,...payer({fullBirthName:'Cher'})})).status,400,'birth name needs a last space to split on');
  assert.equal((await call('/api/local-pay',{intakeId:intake.id,...payer({fullBirthName:'Joël Chue'})})).status,400,'existing ASCII guard still applies to the birth name');
  for(const dob of ['14/03/1961','1961-02-30','2020-01-01','1800-01-01',undefined])assert.equal((await call('/api/local-pay',{intakeId:intake.id,...payer({dateOfBirth:dob})})).status,400,'reject dob '+dob);
  const order=(await call('/api/local-pay',{intakeId:intake.id,...payer({displayFirstName:'Jo',fullBirthName:" Mary Anne O'Connor "})})).data.order;
  ...
  const baseIntake=(await call('/api/intake',{editionId:'commitment-v1',sameDay:false})).data.intake;
  const base=(await call('/api/local-pay',{intakeId:baseIntake.id,...payer({email:'base@example.test',fullBirthName:'Jane Smith'})})).data.order;assert.equal(base.totalCents,3500);assert.notEqual(base.id,order.id);
...
test('restart preserves intakes, paid draws, deadlines, audio and fixture artifacts',async()=>{
 ...
 await start();
 try {
  const {intake}=await call('/api/intake',{editionId:'healing-v1',sameDay:true});
  const results=await Promise.all(Array.from({length:5},()=>call('/api/local-pay',{intakeId:intake.id,...payer({email:'fixture@example.test',fullBirthName:'Jane Smith'})})));
  ...
  const {intake:baseIntake}=await call('/api/intake',{editionId:'commitment-v1',sameDay:false});
  ...
  const replay=await call('/api/local-pay',{intakeId:intake.id,...payer({email:'fixture@example.test',fullBirthName:'Jane Smith'})});assert.equal(replay.status,200);assert.deepEqual(replay.order,completed.order);
...
test('failed payment persistence rolls back order and intake index together',async()=>{
 ...
 try{
  const {intake}=await post('/api/intake',{editionId:'healing-v1',sameDay:false});
...
test('internal fixture operations require service token and accept a full saved edition envelope',async()=>{
 ...
 try{
  const {data:{intake}}=await post('/api/intake',{editionId:'quiet-v1',sameDay:false});
  const {data:{order}}=await post('/api/local-pay',{intakeId:intake.id,...payer({email:'qa@example.test',fullBirthName:'Jane Smith'})});
