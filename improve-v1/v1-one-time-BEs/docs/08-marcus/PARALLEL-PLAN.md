...
**D5 amended 2026-09-14 by Joel: the personal fields move onto the booking page.** Stripe's docs prohibit collecting personal data in Checkout `custom_fields`, and a custom field cannot show faint hint text — so the display first name, full birth name and date of birth (Month · Day · Year, three boxes, the checkout-sim design) are collected on OUR booking page, above the pay button, and parked on `be_order_intake` by `POST /api/backend/checkout` before Stripe (validated there: name 1–60, birth name 1–200, date through the be08Birth parser; a bad date is refused before money, never in a URL or on Stripe metadata). Stripe Checkout now collects email, card and name on card only; `marcus-reading` has no custom fields. The webhook reads the intake first and falls back to `session.custom_fields` only for sessions created before this change; every failure path (missing DOB → support note, never blocks a paid order) is unchanged.
...
|---|---|---|
| **T6 Booking route** (audit §4) | T2, T3, design review 01/04/05 | `client/src/pages/marcus/MarcusBooking.tsx` at `/marcus/reading/:editionId`; loads edition from the exported record (production edition table comes with T9); **⚠ superseded 2026-09-14/15 (Joel): birth name and date of birth are collected ON the booking page, never on Stripe — see the D5-amended paragraph above; the built page has the three boxes.** Original row: no form — cards, price, bump, one button; `saveOrderIntake` (edition + bump only) then `POST /api/backend/checkout` with three `custom_fields` (display first name, full birth name, date of birth); cancel returns to the same page; webhook copies custom fields into `be_order_intake`; server-side DOB validation with a support fallback; `VITE_BACKEND_CHECKOUT_LIVE` preview gate. |
...
| **T11 n8n Stage 2 wiring** (audit §11) | T5, T9, T10 | Target = the **real** workflow `UJamB32MGlNKdoEW` (48-node numerology-anchored draft, built by `build-workflow.py`; its full generation core must be brought in from the tested Stage 1 lane in `build-numerology-workflow.py` — one builder, not a hand edit). Preserve the explicit numerology gate. Replace the manual input node with a Stripe webhook trigger + `GET /api/be/08/fulfilment/:sessionId`; add `the_cross` (5, free 2) and `cross_and_triangle` (7, free 2); drop or re-cut `adaptive_eight` to the ⅓ rule; store PDF to `analysis_pdf/08/<order>/`; sign; `POST /delivered`; keep inactive. |
