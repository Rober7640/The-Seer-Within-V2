...
separate document — see [`SHAPE.md`](../daily-email/SHAPE.md).
...
> card identified from her full birth name, anchored by the Life Path from her date of birth.

Her full birth name identifies a separate personal tarot card; her date of birth gives the Life
Path that the report opens on (see [`n8n/NUMEROLOGY-ANCHORED-READING.md`](../n8n/NUMEROLOGY-ANCHORED-READING.md)).
The paid positions are drawn independently for each buyer; the report interprets those cards
through her personal card’s lens. Booking collects display first name, full birth name, date of
birth and delivery email (decision D1, Joel, 2026-09-13); no personal-sentence box.
...
Booking collects her full birth name and date of birth (plus a display first name and the delivery
email; Stripe's email is authoritative). The letter's close asks for the same two things — name and
date of birth — so the close and the page now agree. No question box, in the letter or on the page.

---

## 3 · Her name, and the empty seat

Collect her **full birth name** (as on her birth certificate) and her **date of birth** on the
booking page, plus a display first name and the delivery email. All are required (D1). The name
gives the card below; the date of birth gives the Life Path, and Stage 1 will not run without it.
...
Two uses, in order of weight (a third, the Significator, is retired):
...
⚠ D1 asks for the birth-certificate name, which sits awkwardly with the line above. Which name
rules apply (married, accents, non-Latin) is decision D7 in [`PARALLEL-PLAN.md`](../PARALLEL-PLAN.md)
§1 and is not settled here.
...
booking page  → same edition; POST display first name, full birth name, date of birth, delivery email into private unpaid intake
...
n8n           → saved order/draw/lens → written report (PDF, signed link) → optional audio → delivery
```
...
- [x] Main written format and delivery: a **PDF behind a signed link**, within **24 elapsed hours**
      of confirmed main payment (12 hours with the bump).
- [x] Timing is elapsed hours from confirmed payment, so there is no same-day cutoff or timezone
      rule. Audio inherits the same deadline.
