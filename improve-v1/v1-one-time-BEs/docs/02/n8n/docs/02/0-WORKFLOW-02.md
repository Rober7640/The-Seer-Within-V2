...
⚠ **And a fourth, added 2026-09-07: nothing delivers.** `markBackendReadingDelivered()`
is written and tested in `server/lib/aweber.ts` and **has no caller** — so even with
C0 done and the flag flipped, a buyer would get her thank-you and never her reading.
The fulfilment workflow and its phased test plan:
[`02-n8n-test-plan.md`](./02-n8n-test-plan.md). ✅ The workflow is built and on the instance
(`docs/02/02-HANDOVER.md`), **arc-guided by letter since 2026-09-09**, and blocked only on the
Supabase bucket at node 12. The test drives stop before that node, so they run green.
