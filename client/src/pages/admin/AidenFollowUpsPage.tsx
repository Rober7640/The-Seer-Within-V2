...
const TAB_META: Record<SequenceType, { title: string; description: string; envFlag: string; flagOnNote: string; flagOffNote: string }> = {
  ...
  post_purchase: {
    title: "Post-Purchase",
    description:
      "Pastoral-care nurture for users on their FIRST completed purchase routed to Aiden. 10 emails at 48h spacing (+2d through +20d). Cron runs every 5 min. Emails are Haiku-personalized at send time using the user's quiz topic and chat history. Pastoral / shadow-work tone — never hard-sell, never discounts. Subsequent purchases do NOT halt the drip. Skips only on unsubscribe, email change, suspension, or staleness.",
                      ...
                      <td colSpan={12} className="p-8 text-center text-gray-600 text-sm">
                        {activeTab === "unverified"
                          ...
                          : activeTab === "verified_nopurchase"
                            ...
                            : "No post-purchase rows yet. Rows are created on a user's FIRST completed purchase routed to Aiden."}
                      </td>
                    </tr>
                  )}
                </tbody>
